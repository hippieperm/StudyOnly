...
<%@page import="java.sql.DriverManager"%> <%@ page language="java"
contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%> <% int
custno=Integer.parseInt(request.getParameter("custno")); String
url="jdbc:oracle:thin:@localhost:1521:xe"; String user="system"; String
password="1234"; Class.forName("oracle.jdbc.driver.OracleDriver"); Connection
conn=DriverManager.getConnection(url, user, password); String sql="select * from
member_tbl_02 where custno=?"; PreparedStatement ps=conn.prepareStatement(sql);
ps.setInt(1, custno); ResultSet rs=ps.executeQuery(); %>
<!DOCTYPE html>
<html>
  <head>
    <meta charset="UTF-8" />
  ...
  <body>
    ...
    <section>
      ...
      <div>
        <% if(rs.next()){ %>
        <form action="edit_f.jsp" method="post" onsubmit="return confirm()">
          <table border="1">
            <tr>
              <td>회원번호</td>
              <td>
                <input type="text" name="custno" id="custno"
                value="<%=rs.getInt("custno")%>">
              </td>
            </tr>
            <tr>
              <td>회원성명</td>
